...
def test_v12_version_and_packaging():
    """Validates pyproject.toml contains correct version and required dependencies."""
    with open("pyproject.toml", "r", encoding="utf-8") as f:
        content = f.read()
    assert re.search(r'version\s*=\s*"1\.2\.\d+"', content) is not None
...
def test_prometheus_metrics_endpoint():
    ...
    assert re.search(r'oracle_system_version\{version="1\.2\.\d+"\}\s+1', text) is not None
