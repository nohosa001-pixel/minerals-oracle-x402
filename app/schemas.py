...
class MinePermitsRecord(BaseModel):
    ...
    simbara_ntpn: Optional[str] = Field(default=None, description="Indonesia SIMBARA/e-PNBP tax payment code (NTPN)")
    simbara_rkab_quota_id: Optional[str] = Field(default=None, description="Indonesia annual approved production quota (RKAB) ID")
    dhe_forex_deposit_ref: Optional[str] = Field(default=None, description="Indonesia Bank Indonesia 30% export forex deposit receipt (DHE)")
    ceec_barcode_tag_id: Optional[str] = Field(default=None, description="DRC CEEC official tamper-evident barcode tag series")
    egc_artisanal_license: Optional[str] = Field(default=None, description="DRC EGC monopoly buying license (for artisanal concessions)")
    chile_dga_water_permit_id: Optional[str] = Field(default=None, description="Chile DGA brine & freshwater extraction permit ID")
    australia_epbc_ref_no: Optional[str] = Field(default=None, description="Australia Federal EPBC Act MNES environmental approval ID")
    argentina_concession_id: Optional[str] = Field(default=None, description="Argentina Provincial mining concession registry ID")
    brazil_anm_license_id: Optional[str] = Field(default=None, description="Brazil ANM mining registry license ID")
    china_dual_use_license_no: Optional[str] = Field(default=None, description="China MOFCOM dual-use export license (Graphite/Antimony)")
    us_bis_scrap_export_license: Optional[str] = Field(default=None, description="US BIS 15 CFR 744 special export license for black mass / tungsten scrap")
    china_extraterritorial_tech_clearance: Optional[str] = Field(default=None, description="China MOFCOM approval for extraterritorial SX refining tech utilization")
    chile_cochilco_export_id: Optional[str] = Field(default=None, description="Chile COCHILCO copper export registration & quota clearance")
    mexico_se_mining_concession: Optional[str] = Field(default=None, description="Mexico Secretaría de Economía mining concession title ID")
    peru_ingemmet_concession_id: Optional[str] = Field(default=None, description="Peru INGEMMET public mining registry concession ID")
...
class EcologicalSpatialRecord(BaseModel):
    ...
    eudr_deforestation_free: bool = Field(default=True, description="Zero deforestation after EUDR cutoff date 2020-12-31")
    periglacial_zone_violation: bool = Field(default=False, description="True if within Argentina Ley 26.639 periglacial protected zone")
...
class LaborHumanRightsRecord(BaseModel):
    child_labor_free_certified: bool = Field(default=True, description="Zero-tolerance verified absence of under-18 workers")
    rmi_rmap_audit_id: Optional[str] = Field(default=None, description="Responsible Minerals Initiative (RMI) RMAP certified audit ID")
    ilua_registration_id: Optional[str] = Field(default=None, description="Australia Native Title Act ILUA registration number")
    forced_labor_uapa_cleared: bool = Field(default=True, description="Verified absence of Xinjiang UFLPA or forced labor ties")
    independent_third_party_auditor: str = Field(default="SGS / Bureau Veritas / RCS Global", description="Third-party audit body")
    csddd_audit_hash: Optional[str] = Field(default=None, description="EU CSDDD supply chain human rights and environmental audit SHA-256 hash")
...
class RefiningMassBalanceRecord(BaseModel):
    ...
    captive_coal_power_used: bool = Field(default=False, description="True if refined with captive coal power (EU CBAM risk)")
    cbam_scope1_emissions_kg_co2e: Optional[float] = Field(default=None, description="Direct Scope 1 emissions in kg CO2e per kg mineral")
    cbam_scope2_emissions_kg_co2e: Optional[float] = Field(default=None, description="Indirect Scope 2 electricity emissions in kg CO2e per kg mineral")
    cbam_scope3_emissions_kg_co2e: Optional[float] = Field(default=None, description="Upstream/downstream Scope 3 emissions in kg CO2e per kg mineral")
    cbam_declaration_id: Optional[str] = Field(default=None, description="EU CBAM Definitive Period registry declaration reference")
    solvent_extraction_tech_origin: Optional[str] = Field(default="DOMESTIC", description="Origin of SX technology: DOMESTIC, WESTERN, CHINA_LICENSED, CHINA_UNLICENSED")
    chinese_tech_dependency_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Percentage dependency on Chinese proprietary SX/smelting technology/catalysts")
    mofcom_extraterritorial_clearance_id: Optional[str] = Field(default=None, description="China MOFCOM dual-use extraterritorial technology export authorization ID")
    substitute_western_tech_certified: bool = Field(default=False, description="Certified availability and operational deployment of non-Chinese Western alternative SX/refining technology")
    sulfuric_acid_input_metric_tons: Optional[float] = Field(default=None, description="Sulfuric acid (H2SO4) input in metric tons for copper leach/smelting")
    sulfuric_acid_discrepancy_pct: Optional[float] = Field(default=None, description="Discrepancy % from stoichiometric sulfuric acid requirement (max allowable 5.0%)")
    hvdc_grade_copper_certified: Optional[bool] = Field(default=None, description="True if copper cathode meets ASTM B115 Grade 1 (99.9935% Cu) for HVDC cables")
    solar_pv_topcon_silver_certified: Optional[bool] = Field(default=None, description="True if silver powder/paste meets 99.99% purity for N-type TOPCon solar PV cells")
...
class MaritimeLogisticsRecord(BaseModel):
    ...
    transshipment_port: Optional[str] = Field(default=None, description="Intermediate transit port (e.g. Singapore, Batam)")
    iso_17025_lab_coa_hash: str = Field(..., description="ISO/IEC 17025 accredited laboratory assay COA certificate hash")
    tml_moisture_safe: bool = Field(default=True, description="IMSBC Code Transportable Moisture Limit (TML) liquefaction safe")
...
class GeopoliticalSanctionsRecord(BaseModel):
    ...
    ofac_sdn_sanctioned: bool = Field(default=False, description="True if associated with OFAC SDN list (e.g. Russia Nornickel)")
...
class SecurityAttestation(BaseModel):
    security_gate_certified: bool = Field(default=True, description="True if verified and certified by Security Gate x402")
    security_gate_url: str = Field(..., description="Endpoint URL of the certifying security gate")
    security_gate_mode: str = Field(default="REMOTE_GATEWAY", description="Verification mode: REMOTE_GATEWAY or LOCAL_STANDALONE")
    agent_address: Optional[str] = Field(default=None, description="Requesting agent wallet address")
    agent_credit_tier: Optional[str] = Field(default=None, description="FICO-style credit tier (e.g. AAA, AA, A, BBB)")
    agent_credit_score: Optional[int] = Field(default=None, description="Dynamic agent credit score (300-850)")
...
class MineralLotProvenanceRequest(BaseModel):
    ...
    is_recycled_black_mass: bool = Field(default=False, description="True if cargo batch originates from shredded battery black mass or secondary scrap")
    us_bis_export_authorized: bool = Field(default=True, description="True if authorized under US BIS 15 CFR 744 scrap retention rules")
...
class ComplianceVerdict(BaseModel):
    ...
    us_bis_scrap_retention_cleared: bool = Field(default=True, description="Compliant with US BIS 15 CFR 744 battery scrap retention mandate (Trap 13)")
    china_tech_jurisdiction_cleared: bool = Field(default=True, description="Compliant with China Extraterritorial SX refining tech rules (Trap 14)")
    china_tech_licensing_cleared: bool = Field(default=True, description="Compliant with China Extraterritorial Tech Licensing Jurisdiction (Trap 15)")
    cbam_definitive_period_verified: bool = Field(default=True, description="Verified under EU CBAM Definitive Period Scope 1-3 rules")
    downstream_cbam_scope3_cleared: bool = Field(default=True, description="Compliant with EU CBAM 2028 downstream finished/machinery product rules")
    csddd_due_diligence_verified: bool = Field(default=True, description="Compliant with EU CSDDD human rights and environmental due diligence audit")
    copper_hvdc_certified: Optional[bool] = Field(default=None, description="Certified for AI Data Center & HVDC Power Grid infrastructure")
    silver_solar_pv_cleared: Optional[bool] = Field(default=None, description="Certified for N-type TOPCon high-efficiency solar PV cells")
    zkp_privacy_sealed: bool = Field(default=True, description="Commercial pricing & supplier contracts blinded via ZKP")
...
class LegalDisclaimerRecord(BaseModel):
    disclaimer_version: str = Field(default="2026.09-v1", description="Legal disclaimer terms version")
    liability_cap_usdc: float = Field(default=0.50, description="Strict maximum liability cap per verification request in USDC")
    warranty_disclaimer: str = Field(
        default="ALGORITHMIC_VERIFICATION_ONLY: This passport represents mathematical and algorithmic rule verification "
    ...
    consequential_damages_waiver: str = Field(
        default="ZERO_CONSEQUENTIAL_LIABILITY: The Oracle and its operators bear zero liability for customs seizures, "
    ...
    non_delegable_audit_duty: str = Field(
        default="NON_DELEGABLE_IMPORTER_DUTY: The recipient retains sole and non-delegable legal obligation to perform "
...
class ResponseMeta(BaseModel):
    license: str = Field(default="AS-IS", description="Open Source AS-IS Software & Data Reference License")
    disclaimer: str = Field(
        default="This output is an automated algorithmic data reference and does not constitute "
    ...
    warranty: str = Field(
        default="PROVIDED 'AS IS', WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING "
        "BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR "
        "PURPOSE AND NONINFRINGEMENT.",
        description="Standard MIT/Apache 2.0 disclaimer of warranty"
    )
    service_nature: str = Field(
        default="Algorithmic Heuristic Calculator & Public Ledger Feed (Stateless M2M)",
...
class CompliancePassportResponse(BaseModel):
    ...
    eip712_signature: Optional[str] = Field(default=None, description="ECDSA EIP-712 structured cryptographic signature")
    onchain_anchor_tx: Optional[str] = Field(default=None, description="Polygon transaction hash if anchored")
...
class AgentRegisterRequest(BaseModel):
    ...
    agent_address: Optional[str] = Field(default=None, description="Optional Polygon wallet address (0x...)")
    initial_trial_balance_usdc: float = Field(default=0.05, ge=0.0, description="Trial balance granted on self-onboarding")
...
class AgentDepositInput(BaseModel):
    identifier: Optional[str] = Field(default=None, description="Agent session key (vault_key_...) or wallet address (0x...)")
    agent_address: Optional[str] = Field(default=None, description="Agent Polygon wallet address (0x...)")
    amount_usdc: float = Field(..., gt=0, description="Amount of USDC to deposit")
    tx_hash: Optional[str] = Field(default=None, description="On-chain settlement transaction hash (optional for simulated deposit)")
    chain: str = Field(default="polygon", description="Settlement chain: polygon, base, arbitrum")
...
class ProcurementRFQRequest(BaseModel):
    ...
    cell_chemistry: str = Field(default="NCM811", description="Target battery cell chemistry (e.g. NCM811, NCM622, LFP)")
    pack_capacity_kwh: float = Field(84.0, gt=0, description="Pack capacity in kWh")
    lithium_tons: float = Field(..., gt=0)
    lithium_origin_country: str = Field(default="AUS")
    lithium_feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0)
    nickel_tons: float = Field(..., gt=0)
    nickel_origin_country: str = Field("IDN")
    nickel_feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0)
    cobalt_tons: float = Field(..., gt=0)
    cobalt_origin_country: str = Field("COD")
    cobalt_feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0)
...
class LithiumOriginVerifyRequest(BaseModel):
    ...
    product: str = Field(default="Lithium Hydroxide Monohydrate", description="Refined product form: Lithium Hydroxide, Lithium Carbonate, Spodumene Concentrate")
    mine_name: str = Field(..., description="Name of hard-rock mine (e.g. Greenbushes Lithium Mine, Pilgangoora)")
    mine_country: str = Field(default="AU", description="ISO alpha-2 country of extraction (AU default)")
    coordinates: List[float] = Field(..., min_length=2, max_length=2, description="[Latitude, Longitude] of extraction centroid")
    minedex_tenement_id: Optional[str] = Field(default=None, description="Western Australia DMIRS MINEDEX tenement ID (e.g., M01/03)")
    spodumene_tonnage_extracted: float = Field(..., gt=0, description="Gross run-of-mine or SC6 spodumene input in metric tons")
    spodumene_grade_pct: float = Field(default=6.0, ge=1.0, le=10.0, description="Spodumene Li2O concentrate grade percentage (standard 6.0%)")
    refinery_facility: str = Field(..., description="Name of refining or chemical conversion plant")
    refinery_country: str = Field(default="AU", description="ISO alpha-2 country of the refinery plant (e.g. AU, US, CHN)")
    refined_output_tonnage: float = Field(..., gt=0, description="Finished lithium product output in metric tons")
    refinery_feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Covered nation entity equity/voting share in refinery")
    satellite_vegetation_index: Optional[float] = Field(default=None, description="Sentinel-2 NDVI for the extraction boundary (typically < 0.2 in active pit)")
    sar_backscatter_db: Optional[float] = Field(default=None, description="Sentinel-1 SAR C-band backscatter in dB")
    agent_address: Optional[str] = Field(default=None, description="Calling agent Polygon wallet address for receipt generation")
...
class NickelOriginVerifyRequest(BaseModel):
    ...
    product: str = Field(default="Nickel Mixed Hydroxide Precipitate (MHP)", description="Beneficiated intermediate product form (MHP or Ferronickel)")
    ...
    dhe_forex_deposit_ref: Optional[str] = Field(default=None, description="Bank Indonesia 30% export forex deposit receipt reference (DHE BI)")
    limonite_ore_input_tons: float = Field(..., gt=0, description="Gross wet/dry laterite limonite ore input in metric tons")
    ore_grade_ni_pct: float = Field(default=1.35, ge=0.5, le=3.0, description="Limonite ore nickel content % (standard 1.2% ~ 1.5%)")
    ...
    mhp_grade_ni_pct: float = Field(default=38.5, ge=30.0, le=45.0, description="MHP nickel metal content % (standard ~38% ~ 40%)")
    captive_coal_power: bool = Field(default=False, description="True if HPAL plant runs on dedicated captive coal-fired power (EU CBAM risk)")
    feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Covered nation entity equity/voting share in smelter JV")
    agent_address: Optional[str] = Field(default=None, description="Calling agent Polygon wallet address for receipt generation")
...
class CobaltOriginVerifyRequest(BaseModel):
    ...
    product: str = Field(default="Crude Cobalt Hydroxide", description="Beneficiated intermediate product form")
    concession_name: str = Field(..., description="Mining concession name (e.g. Tenke Fungurume, Kamoto Copper Company, Mutanda Mining)")
    province: str = Field(default="Lualaba", description="DRC Province (e.g. Lualaba, Haut-Katanga)")
    coordinates: List[float] = Field(..., min_length=2, max_length=2, description="[Latitude, Longitude] centroid of extraction pit")
    mine_type: str = Field(default="LSM", description="Extraction scale: Large-Scale Mining ('LSM') or Artisanal & Small-scale ('ASM')")
    ceec_seal_id: str = Field(..., description="DRC CEEC (Centre d'Expertise) tamper-proof barcode seal / export certificate")
    egc_custody_ref: Optional[str] = Field(default=None, description="Entreprise Générale du Cobalt (EGC) custody reference if ASM involvement")
    asm_comingled: bool = Field(default=False, description="True if uncertified artisanal ore was co-mingled into industrial lot (Trap 1 risk)")
    zero_child_labor_audit_ref: Optional[str] = Field(default=None, description="Independent ILO 138/182 zero child labor verification certificate hash/ID")
    heterogenite_ore_input_tons: float = Field(..., gt=0, description="Gross heterogenite/copper-cobalt ore input in metric tons")
    ore_grade_co_pct: float = Field(default=1.50, ge=0.2, le=10.0, description="Cobalt grade in run-of-mine ore % (standard 1.0% ~ 2.5%)")
    refinery_name: str = Field(..., description="Refinery / hydrometallurgical leaching plant name")
    refinery_country: str = Field(default="COD", description="Country of primary refining (e.g. COD, FIN, CHN)")
    rmi_rmap_smelter_id: Optional[str] = Field(default=None, description="Responsible Minerals Initiative (RMI) RMAP audited smelter ID")
    cobalt_hydroxide_output_tons: float = Field(..., gt=0, description="Refined crude cobalt hydroxide output in metric tons")
    hydroxide_grade_co_pct: float = Field(default=30.0, ge=15.0, le=45.0, description="Cobalt metal content % in hydroxide (standard ~30.0%)")
    feoc_equity_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Covered nation entity equity/voting share in operator (cap < 25%)")
    agent_address: Optional[str] = Field(default=None, description="Calling agent Polygon wallet address for receipt generation")
...
class CompositeBatteryVerifyRequest(BaseModel):
    ...
    cell_chemistry: str = Field(default="NCM811", description="Cathode active material chemistry (e.g. NCM811, NCM622, NCM523)")
    ...
    agent_address: Optional[str] = Field(default=None, description="Requesting OEM/tier-1 buyer Polygon wallet address")
...
class CopperOriginVerifyRequest(BaseModel):
    ...
    mine_concession_name: str = Field(default="CHUQUICAMATA", description="Canonical mine concession: CHUQUICAMATA, EL_TENIENTE, ANDINA, ESCONDIDA, LOS_PELAMBRES, CERRO_VERDE")
    ...
    concentrate_grade_cu_pct: float = Field(default=28.0, ge=10.0, le=45.0, description="Copper concentrate assay grade % (default 28.0% Cu)")
    ...
    copper_cathode_purity_pct: float = Field(default=99.9935, ge=99.0, le=100.0, description="Cathode assay purity % (ASTM B115 Grade 1 requires 99.9935%)")
    cochilco_export_clearance_id: Optional[str] = Field(default=None, description="Chile COCHILCO export quota and registration ID")
    hvdc_cable_spec_compliant: bool = Field(default=True, description="True if certified for AI Data Center HVDC power grid transmission")
    feoc_shareholding_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Covered nation equity shareholding %")
    agent_address: Optional[str] = Field(default=None, description="Buyer agent Polygon wallet address")
...
class SilverOriginVerifyRequest(BaseModel):
    ...
    mine_concession_name: str = Field(default="TERRONERA", description="Canonical mine concession: TERRONERA, FRESNILLO, ANTAMINA, UCHUCCHACUA, LOS_PELAMBRES")
    extraction_coordinates: Tuple[float, float] = Field(..., description="(lat, lon) centroid coordinates")
    ...
    feedstock_silver_grade_pct: float = Field(default=75.0, ge=10.0, le=98.0, description="Doré bullion silver content % (default 75.0% Ag)")
    refined_solar_powder_kg: float = Field(..., gt=0, description="Finished electrolytic silver powder/paste yield in kg")
    refined_purity_pct: float = Field(default=99.99, ge=95.0, le=100.0, description="Refined silver assay purity % (Solar PV paste requires >= 99.99%)")
    lbma_good_delivery_ref: Optional[str] = Field(default=None, description="LBMA Good Delivery ref or accredited refiner license ID")
    conflict_free_asm_verified: bool = Field(default=True, description="Verified free from illegal artisanal laundering or cartel supply")
    topcon_pv_grade_compliant: bool = Field(default=True, description="Certified for N-type TOPCon / HJT solar cell metallization paste")
    feoc_shareholding_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Covered nation equity shareholding %")
    agent_address: Optional[str] = Field(default=None, description="Buyer agent Polygon wallet address")
...
class SMELightweightInput(BaseModel):
    ...
    business_registration_no: Optional[str] = Field(default=None, description="Tax or corporate registration identifier")
    ...
    scrap_recycled_ratio_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Percentage of secondary recycled scrap utilized")
    monthly_electricity_kwh: float = Field(..., ge=0.0, description="Monthly grid electricity consumption from utility invoice (kWh)")
    grid_region: str = Field(default="KR_GRID", description="Grid emission region: KR_GRID, US_GRID, EU_GRID, CL_GRID")
    purity_pct: float = Field(default=99.99, ge=80.0, le=100.0, description="Finished product assay purity percentage")
...
class SupplyChainBatchRequest(BaseModel):
    ...
    batch_title: str = Field(default="Tier-Supplier Global Compliance Batch", description="Audit batch run description")
...
class HSCodeTariffInfo(BaseModel):
    ...
    fta_name: Optional[str] = Field(default=None, description="Applicable Free Trade Agreement")
    section_301_tariff_pct: float = Field(default=0.0, description="US Section 301 punitive tariff % (e.g. on Chinese goods)")
    export_licensing_required: bool = Field(default=False, description="True if origin country requires dual-use/export license")
    export_restriction_note: Optional[str] = Field(default=None, description="Export policy note (e.g. MOFCOM or Indonesia DMO)")
    eu_cbam_applicable: bool = Field(default=False, description="True if subject to EU Carbon Border Adjustment Mechanism")
    cbam_default_carbon_intensity: float = Field(default=0.0, description="Default embedded carbon benchmark (tCO2e/t)")
...
class MaritimeRouteRequest(BaseModel):
    ...
    cargo_weight_metric_tons: float = Field(default=1000.0, ge=1.0, description="Shipment lot weight in metric tons")
    vessel_imo_number: Optional[int] = Field(default=None, description="IMO 7-digit vessel ID")
    cii_rating: MaritimeCIIRating = Field(default=MaritimeCIIRating.A, description="Vessel IMO MARPOL Carbon Intensity rating")
...
class TradeRouteOptimizationRequest(BaseModel):
    ...
    target_delivery_deadline_days: Optional[float] = Field(default=None, description="Max acceptable transit time in days")
    max_carbon_budget_co2_tons: Optional[float] = Field(default=None, description="Max acceptable voyage CO2 in tons")
...
class AgentDecisionSignal(BaseModel):
    ...
    projected_cost_delta_usd: float = Field(default=0.0, description="Projected dollar savings (positive) or penalty (negative)")
    actionable_command: Optional[str] = Field(default=None, description="Ready-to-execute next MCP tool call snippet")
...
class AgentSessionOpenRequest(BaseModel):
    ...
    session_duration_hours: int = Field(default=24, ge=1, le=168, description="Session validity in hours (1-168)")
    signature: Optional[str] = Field(default=None, description="Optional EIP-712 deposit authorization signature")
...
class TradeDealSpec(BaseModel):
    ...
    feoc_cleared: bool = Field(default=True, description="True if certified < 25% covered nation equity")
    mass_balance_cleared: bool = Field(default=True, description="True if certified under stoichiometric mass balance")
    ...
    settlement_currency: str = Field(default="USDC", description="Settlement cryptocurrency")
    projected_savings_usd: float = Field(default=0.0, description="Oracle-calculated tariff and logistics savings in USD")
    gain_share_rate_pct: float = Field(default=10.0, description="Agreed gain-share percentage (default 10%)")
    calculated_gain_share_fee_usd: float = Field(default=0.0, description="Calculated gain-share settlement fee in USD")
    fee_cap_applied: bool = Field(default=False, description="True if fee exceeded cap (e.g. $10,000 max)")
    hybrid_settlement_summary: Optional[str] = Field(default=None, description="Detailed hybrid fee breakdown")
    expires_at_utc: Optional[str] = Field(default=None, description="Expiration timestamp in ISO UTC (default: 24h after creation)")
    created_at_utc: str
...
class TradeDealRejectRequest(BaseModel):
    deal_id: str = Field(..., description="Deal ID to reject")
    buyer_agent_address: str = Field(..., description="Buyer agent address executing rejection")
    buyer_signature: Optional[str] = Field(default=None, description="Cryptographic signature confirming rejection")
    rejection_reason: str = Field(default="TERMS_UNACCEPTABLE", description="Reason for rejection")


class TradeDealCancelRequest(BaseModel):
    deal_id: str = Field(..., description="Deal ID to cancel/revoke")
    seller_agent_address: str = Field(..., description="Seller agent address executing revocation")
    seller_signature: Optional[str] = Field(default=None, description="Cryptographic signature confirming cancellation")
    cancellation_reason: str = Field(default="SELLER_WITHDRAWAL", description="Reason for cancellation")


class TradeDealAttestation(BaseModel):
    deal_id: str
    status: str = Field(default="DUAL_SIGNED_CONFIRMED", description="Deal status: PROPOSED, DUAL_SIGNED_CONFIRMED, REJECTED, CANCELLED, EXPIRED")
    deal_hash: str
    spec: TradeDealSpec
...
class TradeDealListResponse(BaseModel):
    status: str = "success"
    total_count: int
